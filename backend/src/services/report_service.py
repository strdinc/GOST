...
def build_simple_markdown_report(payload: dict[str, Any]) -> str:
    ...
    now = dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    lines = [
        "# Упрощенный отчет по решению",
        "",
        f"- Дата: {now}",
        "",
        "В этой версии оставлены только сами действия в двоичной записи. Таблицы, полиномиальные формы и промежуточный hex специально убраны, чтобы отчет читался проще.",
        "",
        "## Исходные данные",
        "",
